...
testCSParser = do 
    describe "Lib.run_parseType" $
        it "returns a Class from a .cs file" $
        run_parseType someClass "(Test)" `shouldBe` Right (Class { class_usings = ["System","System.Collections.Generic","System.Security","Core.Constants","Core.Models.Shared","Core.Queries","Core.Queries.Shared","Core.User","DataAccess.Bruger","DataAccess.Database","System.Linq","AutoMapper.QueryableExtensions"]
                                                                  ...
                                                                  , name = GenericClassName "SagQuery" "T"
                                                                  , class_abstract = True
    ...
    describe "Lib.parseProperty -> single property" $
      it "returns a single" $
    ...
    describe "Lib.parseProperty -> single Property with get set" $
      it "returns a single" $
    ...
    describe "Lib.parseProperty -> Array property" $
      it "returns a list" $
    ...
    describe "Lib.parseProperty -> Array Property with space" $
      it "returns a list" $
        run_test parseProperty "private static readonly Datatype [] _name;" `shouldBe` (Right (Property Nothing (List (Single "Datatype")) (PropertyName "_name") Nothing "" Private Readonly Static []))

    describe "Lib.parseProperty -> Array Property with attribute" $
      it "returns a single with attribute" $
    ...
    describe "Lib.parseProperty -> Property multiple declarations" $
      it "returns a Single with long name" $
    ...
    describe "Lib.parseMember -> Property with curly on new line" $
      it "returns property" $
    ...
    describe "Lib.parseAbstractMethod -> abstract method" $
      it "returns abstract method" $
    ...
    describe "Lib.parseAbstractMethod -> generic method" $
      it "returns generic method" $
    ...
    describe "Lib.parseAbstractMethod -> generic returntype method" $
      it "returns generic returntype method" $
    ...
    describe "Lib.parseMethod -> method with attribute" $
      it "returns method with attribute" $
    ...
    describe "Lib.parseMethod -> method without Parameter NoRef NoParamss" $
      it "returns method without Parameter NoRef NoParamss" $
    ...
    describe "Lib.parseMethod -> method without Parameter NoRef NoParamss and with attribute" $
      it "returns method without Parameter NoRef NoParamss" $
    ...
    describe "Lib.parseMethod -> method without Parameter NoRef NoParamss with curlys" $
      it "returns method without Parameter NoRef NoParamss" $
    ...
    describe "Lib.parseMethod -> method with T return type and constraints" $
      it "returns method" $
    ...
    describe "Lib.parseMethod -> method with out parameter" $
      it "returns method" $
    ...
    describe "Lib.parseContent -> content with multi curlys" $
      it "returns all content" $
        run_test (parseContent) "{ for{{}} }" `shouldBe` (Right "{ for{{}} }")

    describe "Lib.parseConstructor -> constructor" $
      it "constructor" $
...
\        }" `shouldBe` (Right (Constructor Public [Parameter NoRef NoParams (Single "DatabaseDataContext") "context" Nothing False
    ...
    describe "Lib.parseConstructor -> constructor with this : (test)" $
      it "return CtorCall" $
...
\}"))
    ...
    describe "Lib.parseConcrete -> method default Parameter NoRef NoParams" $
      it "returns method" $
    ...
    describe "Lib.parseConcrete -> extension method" $
      it "returns method" $
    ...
    describe "Lib.parseEnum -> enum" $
      it "returns enum" $
        run_test (parseEnum [] []) "public enum DataLocation\n\
                                      \{\n\
                                      \    Host,\n\
                                      \    Device\n\
                                      \}" `shouldBe` (Right (Enum [] [] Public (ClassName "DataLocation") ["Host", "Device"] []))
                                      
    describe "Lib.parseOperatorOverload -> Operator overloading method" $
      it "returns method" $
