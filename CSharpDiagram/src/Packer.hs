...
pack' c@(Packed east south m) (p:ps) = 
    let
        ...
        (height, index) = minimum $ map (\ (e, i) -> (absoluteHeight e, i)) $ (zip (c:allEast) [0..])
        goEast = width < height
    in
...
travelPacked :: (a -> b) -> (b -> b -> b) -> (b -> b -> b) -> Packed a -> b
travelPacked f fe fs p@(Packed (Just e) (Just s) m) = (fe (fs (f m) (travelPacked f fe fs s)) (travelPacked f fe fs e))
travelPacked f fe fs p@(Packed Nothing (Just s) m) = (fs (f m) (travelPacked f fe fs s))
travelPacked f fe fs p@(Packed (Just e) Nothing m) = (fe (f m) (travelPacked f fe fs e))
travelPacked f fe fs p@(Packed Nothing Nothing m) = f m